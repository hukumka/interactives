...
use lexer::TokenData;
use syntax_tree::{
    BinaryOperator, Block, Condition, Expression, ExpressionData, ForLoop, ForLoopInitialization,
    FunctionCall, FunctionDeclaration, FunctionDefinition, Index, PrefixOperator, Root, Statement,
    SyffixOperator, TreeItem, VariableDefinition,
};
use types::{BaseType, FunctionType, Type};
...
impl<'a> VariableManager<'a> {
    ...
    fn register_variable(
        ...
        name: &'a TokenData<'a>,
        type_: Type,
    ) -> Result<VariableHandleAddress, Error<'a>> {
    ...
    fn get_function_id(&self, name: &'a TokenData<'a>) -> Option<usize> {
        match self.globals.get(name.token_str()) {
            Some(VariableHandle {
                address: VariableHandleAddress::Global(x),
                ..
            }) => Some(*x),
            _ => None,
        }
...
impl DebugInfo {
    ...
    fn push_variable<'a>(
        ...
        if let VariableHandleAddress::Local(_) = stack_offset {
            self.variables_transactions_stack
                .push(self.variables_transactions.len());
    ...
    fn reg_statement_offset<'a>(&mut self, statement: &'a Statement<'a>, vm_offset: usize) {
        ...
        self.statements
            .insert(statement.first_token().get_pos(), vm_offset);
    }

    /// return data about variable use
    pub fn get_variable_data<'a>(
        &self,
        name: &'a TokenData<'a>,
    ) -> Option<(VariableHandleAddress, usize)> {
    ...
    pub fn get_statement_offset<'a>(&self, statement: &'a Statement<'a>) -> Option<usize> {
        self.statements
            .get(&statement.first_token().get_pos())
            .cloned()
    }
...
impl<'a> Compiler<'a> {
    ...
    pub fn compile(
        mut self,
        tree: &'a [Root<'a>],
    ) -> Result<(Vec<Operation>, DebugInfo), Vec<Error<'a>>> {
                ...
                Root::FunctionDeclaration(decl) => {
                    match self.variables.register_function_declaration(decl) {
                        Ok(i) => {
                            self.debug_info
                                .function_links
                                .push((i, decl.ret_name.name.token_str().to_string()));
                        }
                        Err(e) => {
                            self.errors.push(e);
                        }
    ...
    pub fn compile_function(&mut self, func: &'a FunctionDefinition<'a>) -> Option<usize> {
        ...
        self.debug_info
            .functions
            .push((function_id, self.operations.len()));
    ...
    fn compile_return(&mut self, ret: &'a Expression<'a>) -> Option<()> {
        let required_type = Type::from_type(&self.current_function.unwrap().decl.ret_name.type_)
            .or_else(|e| {
                self.errors.push(Error::from_token(
                    ERROR_COMPILER_UNSUPPORTED_TYPE,
                    e.first_token(),
    ...
    fn compile_for_loop(
        ...
        self.variables.pop_namespace().unwrap();
        self.debug_info
            .recover_to_stack_floor(self.operations.len(), stack_floor);
        succ
    }
    ...
    fn compile_expression_variable(&mut self, var: &'a TokenData<'a>) -> Option<(Type, bool)> {
        let variable = if let Some(v) = self.variables.get_variable(var.token_str()) {
            ...
            self.errors
                .push(Error::from_token(ERROR_COMPILER_UNDEFINED_VARIABLE, var));
            return None;
        };
    ...
    fn compile_expression_constant(&mut self, c: &'a TokenData<'a>) -> Option<(Type, bool)> {
        ...
        if let Ok(i) = c.token_str().parse::<i32>() {
            ...
            self.errors
                .push(Error::from_token(ERROR_COMPILER_INVALID_CONSTANT, c));
            None
        }
    }
    ...
    fn compile_expression_function_call(
        &mut self,
        func: &'a FunctionCall<'a>,
    ) -> Option<(Type, bool)> {
        let type_ = self.compile_expression_rvalue(&func.func)?;
        let type_ = match type_ {
            Type {
                pointer_count: 0,
                base: BaseType::Function(box func),
                ..
            } => func,
            _ => {
                self.errors.push(Error::from_token(
                    ERROR_COMPILER_EXPECTED_FUNCTION,
                    func.first_token(),
                ));
                return None;
            }
        ...
        if func.arguments.len() == type_.args.len() {
            ...
            self.errors.push(Error::from_token(
                ERROR_COMPILER_MISMATCHED_ARGUMENTS_COUNT,
                func.first_token(),
            ));
            None
        }
    ...
    fn compile_expression_binary_op(
        &mut self,
    ...
    fn compile_less_operator(
        ...
        if Self::can_compare(l, r) {
            ...
            self.errors
                .push(Error::from_token(ERROR_COMPILER_MISMATCHED_TYPES, token));
            None
        }
    }

    fn compile_greater_operator(
        &mut self,
        token: &'a TokenData<'a>,
        l: &Type,
        r: &Type,
    ...
    fn compile_less_or_eq_operator(
        &mut self,
        token: &'a TokenData<'a>,
        l: &Type,
        r: &Type,
    ...
    fn compile_greater_or_eq_operator(
        &mut self,
        token: &'a TokenData<'a>,
        l: &Type,
        r: &Type,
    ...
    fn compile_operator_minus(
        ...
        if l.pointer_count > 0 && r == &Type::int() {
            ...
            Some((l.clone(), false))
        } else if (l.pointer_count > 0 && l == r) || (l == &Type::int() && r == &Type::int()) {
            self.operations.push(Operation {
                value: None,
                code: Code::Diff,
                args: vec![res, left, right],
    ...
    fn is_convertion_free(from: &Type, into: &Type) -> bool {
        from == into || from.pointer_count > 0 && into == &Type::int() || from.autocast(into)
    }
