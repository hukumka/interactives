...
use syntax_tree::{
    FunctionDeclaration, FunctionDefinition, FunctionType as FunctionTypeIm, Type as TypeIm,
...
impl FunctionType {
    ...
    pub fn from_declaration<'a>(f: &'a FunctionDeclaration<'a>) -> Result<Self, &'a TypeIm<'a>> {
        ...
        let args: Result<Vec<_>, &'a TypeIm<'a>> = f
            .arguments
            .iter()
            .map(|v| Type::from_type(&v.type_))
            .collect();
        let args = args?;
        Ok(Self { ret, args })
    }
    ...
    pub fn from_func_type<'a>(f: &'a FunctionTypeIm<'a>) -> Result<Self, &'a TypeIm<'a>> {
        ...
        let args: Result<Vec<_>, &'a TypeIm<'a>> =
            f.args.iter().map(|v| Type::from_type(v)).collect();
        let args = args?;
        Ok(Self { ret, args })
    }
}
...
impl Type {
    ...
    pub fn from_type<'a>(t: &'a TypeIm<'a>) -> Result<Self, &'a TypeIm<'a>> {
        Ok(Self {
            base: Self::from_base(t)?,
            pointer_count: t.pointer_count as u8,
            final_: false,
        })
    }
