...
impl<'a> Preprocessor<'a> {
    ...
    fn get_char_iter(&self) -> CharsIter<'a> {
        self.code_text.char_indices().peekable()
    }
    ...
    fn eat_token(&self, char_iter: &mut CharsIter<'a>) -> Option<Result<TokenData<'a>, Error<'a>>> {
        loop {
            ...
            if let Some((pos, f)) = first {
                // guarantees what iterator has at least 1 character, so non of functions will panic
                return Some(if f.is_alphabetic() || f == '_' {
    ...
    fn eat_name(&self, char_iter: &mut CharsIter<'a>) -> Result<TokenData<'a>, Error<'a>> {
        Ok(TokenData::new(
            self.code_text,
            self.get_matching_range(char_iter, |x| {
                x.is_alphabetic() || x.is_digit(10) || x == '_'
            }),
            TokenType::Name,
        ))
    }
    ...
    /// # Panics
    /// panic if `char_iter` iterator is empty
    fn get_matching_range<Func: Fn(char) -> bool>(
        &self,
        char_iter: &mut CharsIter<'a>,
        filter: Func,
    ) -> (usize, usize) {
...
mod tests {
    ...
    fn test_mixed() {
        ...
        let data: Vec<_> = data
            .iter()
            .map(|td| (td.type_.as_str(), td.token_str(), td.interval))
            .collect();
        assert_eq!(
            &data,
            &[
    ...
    fn bench_tokenize(bench: &mut Bencher) {
        let preprocessor = Preprocessor::new(
            "int x=0; int x(int a){return a + 4; for(int i=0; i<a; ++i) a += 3 * 4;}",
        );
        bench.iter(|| {
